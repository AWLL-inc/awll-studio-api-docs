...
> `awll.users.{searchMentionable, lookup}` のシグネチャ・引数・制約（権限 / 件数上限 / テナント分離）の詳細は [screen-sdk-reference.md#awllusers--ユーザー検索表示名解決](./screen-sdk-reference.md#awllusers--ユーザー検索表示名解決) を参照。
...
### URL の許可形式 (Backend `@Pattern` + SDK `validateReturnUrl`)
...
| カスタム Screen | `/business/screens/dailyReport?id=A001` | ✅ |
| 絶対 URL | `https://example.com/projects/123` | ❌ (UI 側が cross-origin を silent ブロックするため契約整合化) |
| protocol-relative | `//evil.example.com/x` | ❌ (open redirect 拒否) |
| 制御文字 | `/path\r\nheader: x` | ❌ (CRLF injection 拒否) |
| 危険なスキーム | `javascript:alert(1)` | ❌ (XSS 拒否) |

> **重要**: 同一 origin への遷移は `/path` の相対パスで十分。Backend と UI の入力契約整合のため、API/SDK ともに **絶対 URL `http(s)://...` は不可** (マージ前レビュー対応 2026-05-26)。
...
await awll.comments.create({
  ...
  // 相対パスを指定 (絶対 URL は不可。受信者が踏んだ際に開く画面の URL)
  returnUrl: `/business/screens/projects/${projectId}/ai-reviews/${reviewId}`,
});
```
