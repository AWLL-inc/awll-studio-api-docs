...
> **画面コード（Screen SDK ブリッジ経由）・フロントエンド本体の両方で使用できます。** 一覧画面で多数レコードのサブテーブルを集計する際は、`useNodes` を個別ループ（N+1）せず、本フックで 1 リクエストに集約してください。
...
サブテーブル（ARRAY型フィールド）のデータ操作で、過去にデータ消失インシデントが発生しています。以下のルールを厳守してください。
