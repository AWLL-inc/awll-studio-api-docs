...
| GET | `/api/v1/users/mentionable` | メンション候補ユーザー検索 | 認証済み + 対象form閲覧権限 |
| POST | `/api/v1/users/lookup` | ユーザー一括解決（displayName解決用） | 認証済み |
...
指定されたデータベースのレコード閲覧権限を持つテナント内ユーザーを返します。`<MentionInput>` / TextField / MarkdownField の `@` サジェスト候補に使用します。

呼出元自身も対象 `formId` の閲覧権限を持つ必要があり、返却されるユーザーも同じ閲覧権限を持つユーザーに限定されます（テナント内に限定）。

#### クエリパラメータ

| パラメータ | 型 | デフォルト | 最大 | 説明 |
|-----------|-----|-----------|------|------|
| formId | string | - | - | 対象データベース ID（必須） |
| q | string | - | - | 名前 / email の部分一致クエリ（任意） |
| limit | integer | 20 | 50 | 最大件数 |
...
|-----------|------|
| 400 | formId 未指定 |
| 403 | 呼出元が formId の閲覧権限を持たない |

---

### POST /api/v1/users/lookup

指定された userId リストから displayName / username / email を一括取得します。`<MentionDisplay>` の displayName 解決などに使用します。テナント外 / 存在しない ID はレスポンスから除外されます。

- 最大 200 IDs / リクエスト
- レスポンスは `userId → ユーザーサマリー` のマップ
...
> **重要**: 画面権限とデータベース権限は**別のAPIエンドポイント・別のデータストア**で管理されています。画面にBUSINESS_ACCESSを付与する場合は、必ずこちらのAPIを使用してください。`POST /api/v1/permissions/grant`（データベース権限用）にscreenIdを指定しても画面権限は付与されません。
...
> **前提**: 認証済みセッションで呼び出すこと。本エンドポイントは認証済みユーザーのレコードがバックエンドにプロビジョニング済みであることを前提とします。フロントエンド経由の通常フローでは `/api/auth/me` が先に呼ばれてプロビジョニングが完了しますが、直接 API を叩く場合は **先に `/api/auth/me` を 1 回呼び出してください**。未呼び出しのまま accept すると 404 `UserNotProvisioned` を返します。
...
| Status | 説明 |
|--------|------|
| 400 | 無効なトークン（期限切れ・既に承認済み等） |
| 401 | 認証エラー（JWT 不正・未認証） |
| 403 | 招待の受取人と認証ユーザーのメールアドレスが不一致（招待されたメールアドレスでサインインし直してください） |
| 404 | ユーザー未プロビジョニング（`/api/auth/me` を先に呼び出してください） |
