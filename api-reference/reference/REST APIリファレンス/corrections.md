...
一覧画面は `answerData`（検索インデックス）を参照している。Nodes API で作成したデータはノードツリーには追加されるが、answerData には反映されない。

### 正しい方法

ARRAYフィールドのデータは、**answerData に配列として直接含めて** POST（作成）または PUT（更新）する。

```json
// ✅ 正しい方法: answerData に ARRAY を直接書く
...
// ❌ 間違った方法: 空レコード作成後にNodes APIで個別追加
// → ノードは作成されるが一覧画面に表示されない
```
