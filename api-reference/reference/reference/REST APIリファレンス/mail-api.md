...
| cc | string[] | No | 最大50件、各要素はメールアドレス形式 | CC 宛先リスト（指定要素ごとにメールアドレス形式を検証。不正な場合は 400） |
| bcc | string[] | No | 最大50件、各要素はメールアドレス形式 | BCC 宛先リスト（指定要素ごとにメールアドレス形式を検証。不正な場合は 400） |

> **変更履歴 (2026-04-21 / PR #1642)**: `cc` / `bcc` フィールドを追加しました。各要素はサーバ側でメールアドレス形式を検証（簡易正規表現）し、不正アドレスまたは件数超過（51件以上）は 400 Bad Request を返します。
...
|------|------|
| 送信先 (`to`) | 1リクエスト1宛先 |
| CC (`cc`) | 最大50件（各要素はメールアドレス形式） |
| BCC (`bcc`) | 最大50件（各要素はメールアドレス形式） |
| 件名 | 最大200文字 |
| 本文 | 最大10,000文字 |
