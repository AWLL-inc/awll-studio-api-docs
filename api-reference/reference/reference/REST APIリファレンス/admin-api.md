...
| `roleId` | number | **必須** | 付与するロールID（2026-04-22 以降は必須。未指定時は 400 バリデーションエラー） |
| `username` | string | 任意 | ユーザー名（100 文字以内） |
| `message` | string | 任意 | 招待メールに添えるメッセージ（500 文字以内） |

> **変更履歴 (2026-04-22 / PR #1747)**: `roleId` を必須項目に変更しました。未指定のまま招待された結果として権限が空のままになる事故を防ぐため、未指定リクエストは 400 エラーで拒否されます。
