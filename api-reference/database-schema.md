...
| 種別 | 関数 | 説明 | 例 |
|------|------|------|-----|
| 集約（配列） | `SUM(array[*].field)` | 合計 | `SUM(line_items[*].subtotal)` |
| 集約（配列） | `AVG(array[*].field)` | 平均（数値 0 件なら 0.0） | `AVG(scores[*].value)` |
| 集約（配列） | `COUNT(array[*].field)` | 非 null 件数 | `COUNT(line_items[*].id)` |
| 単項 | `FLOOR(x)` | 切り捨て | `FLOOR(price * 0.85)` |
| 単項 | `CEIL(x)` / `CEILING(x)` | 切り上げ | `CEIL(amount / 100) * 100` |
| 単項 | `ABS(x)` | 絶対値 | `ABS(actual - planned)` |
| 可変 | `ROUND(x)` | 整数四捨五入（half-up） | `ROUND(2.5)` → `3` |
| 可変 | `ROUND(x, n)` | 小数点以下 n 桁で四捨五入 | `ROUND(2.355, 2)` → `2.36` |
| 二項 | `MIN(a, b)` | 最小値 | `MIN(stock, requested)` |
| 二項 | `MAX(a, b)` | 最大値 | `MAX(0, balance)` |

> **`ROUND` の挙動**: `ROUND(x)` は half-up（2.5 → 3）。`ROUND(x, n)` は BigDecimal による精度保証された四捨五入のため、`ROUND(2.355, 2)` は期待通り `2.36` を返します。

> **計算失敗時**: 数式の評価が失敗（未定義関数・配列パス不整合等）してもデータベース保存は中断されず、該当フィールドのみ `null` で保存されます。
